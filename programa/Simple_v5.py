...
from Clases import Mapa, Puerto
from random import random, triangular


class Simulacion:
    def __init__(self, iteraciones, d1):
    ...
    def ejecutar_dia(self, dia, t):
        if len(self.barcos_atracados) > 0:
            desc = 0
            self.revisar_eventos(t)
            while self.eventos and t <= 24 * 60 * dia:
                if self.eventos[0][1] <= 24 * 60 * dia:
    ...
    def corrida(self):
        ...
        while self.barcos_despachados != 16:
            self.iniciar_dia()
            t = self.ejecutar_dia(self.dia, t)
            self.acabar_dia()
        ...
        for barco in self.esperando_atracar:
            if len(self.esperando_atracar) > 0:
                if len(self.barcos_atracados) < 3 and \
                    ...
                    print(f"Atracó el {barco.codigo} en el día {self.dia}.")
    ...
    def calc_min(self, barco):
        return ((barco.partida + 1) - self.dia)*24*60

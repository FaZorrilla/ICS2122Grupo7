...
def construirCamino(etiquetas, origen, destino):
    ...
    if origen == destino:
        return [origen]
    return construirCamino(etiquetas, origen, anterior(etiquetas[destino])) + [
        destino]
...
def generarEtiqueta(grafo, nodo, anterior, etiquetas):
    ...
    etiquetaPropuesta = peso(grafo, anterior, nodo) + acumulado(
        etiquetaNodoAnterior), anterior
    if (nodo not in etiquetas.keys() or acumulado(
            etiquetaPropuesta) < acumulado(etiquetas[nodo])):
